...
**A dragged tile has three places to land, and they are ranked**: the workspace's own outer band (a new column or row beside the whole layout), a split's gutter (between the two tiles it holds), and a tile (its edges split it, its middle swaps). The arbitration is `WorkspaceView` and nowhere else — it is the only control in a workspace carrying `DragDrop.AllowDrop`, because a tile deciding for itself would have to be overruled afterwards by whatever ranked the three, which is two writers for one hint. The outer band is 28px and **outranks the tile underneath**, which is forced rather than chosen: the workspace's padding is eight pixels on three sides and nothing on the left, where the gap is the panel's own splitter column, so a band living only in the padding would have no left edge at all. See [`docs/TILES.md`](docs/TILES.md) → *Where a dragged tile can land*.
