...
**Not every directory without a repository gets the offer.** `SpecialDirectories.AllowsRepository` withholds it for the user's home directory, the root of a drive and the system directories: a repository at `~` tracks every download and every application's configuration, and its first `git status` takes minutes — that is a mistake to make, not a step to be missing. The row simply says nothing there. The rule is pure and shared, because the same "is this the home directory" also decides the workspace's name.
