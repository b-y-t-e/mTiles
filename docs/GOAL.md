...
decided against changing anything. `ClaudeToolRunner.ParseLine` now emits `AiChunkKind.Denied` for
...
of your business" to a model that cannot tell it from a question that was skipped. The panel is capped
and scrolls, because the point of it is that the conversation stays readable while it is up.

The questions are **persisted** (`GoalTileState.PendingQuestions`) — **and so is each answer typed against them**, on the store's own debounce, because coming back with the boxes emptied keeps half of that promise and loses typed text, which is the one thing the rest of this tile refuses to do — and they reach the transcript **when they
are answered**, together with the answer. That keeps the record reading question-then-answer exactly as
it always did, without spending the screen on a second copy of what the panel is already showing —
and it is what lets a tile be closed mid-question and come back still asking. It also moved a rule:
