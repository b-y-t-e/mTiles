...
**What the review is asked to do.** Everything above is what the answer must *look* like; this is the
method, and until recently the prompt had none. Read back from this repository's own goal logs, that
gap is measurable: twenty-one reviews at `effort Medium` returned one finding or none, **eight of them
empty**, against twenty-one at `High` on another machine where not one came back empty and the median
was four. The shortest of the empty ones reads, in full: *"Looks consistent. Build succeeds, all 2350
tests pass."* — which is a faithful execution of the only instruction the prompt actually gave.

Four things close it, and none of them is pressure to find more.

- **Read the change.** The working tree block is named as a map rather than as the evidence, and the
  reviewer is told to open the files it lists. The clause that earns its place at every prompt size is
  the one about new files: no form of `git diff HEAD` carries an untracked file's contents, so on every
  unscoped path — a goal detected from the working tree, the Review button, the review of a tree an
  attempt left alone — the file at the centre of the change arrived as a filename and nothing else.
  Measured on two reviews of the same goal twenty minutes apart: the one that thought to open the new
  class by itself wrote 1 784 characters of analysis, the one that did not wrote the sentence quoted
  above. Which of the two you got was luck. **Deliberately not solved by putting the contents in the
  prompt instead** — `git add -N` against a private index was the first plan, and the evidence is
  against it twice: review quality falls off a cliff as the diff grows (F1 0.80 at ten lines, 0.04 past
  fifty), and on a two-hundred-file change there is no room to grow into, so new content would only
  push tracked changes out of a block that is already a fragment.
- **Work down a list.** Ten categories — correctness, tests, error handling, lifetimes, concurrency,
  security, performance, fit with the project, refactoring safety, and the quality rules already
  stated. Four of those had never been named in this prompt in any form. The published comparison of
  "report the primary issue" against "enumerate what you find" moves the same model from 1.0 findings
  per review to 3.1 and resolves 27% more cases, and attributes the gap to the prioritisation
  constraint hiding issues the model had **already identified**. The first sentence of the list is the
  half that keeps it honest and is not politeness: a category with nothing in it is skipped *silently*,
  because ten headings with no way to answer "nothing here" is an invitation to write something under
  each, and "nothing to report" ten times over is the same padding by another route.
- **Ask what is not there.** A missing test, a migration, documentation describing the old behaviour, a
  related place that should have moved — a whole class of finding a diff cannot show, because an
  absence leaves no line in one. The weighing clause is load-bearing: judged by the fact that something
  is absent, every gap is worth reporting and the review becomes a wish list; judged by what happens
  without it, most gaps are worth nothing and the few that matter carry their real weight.
- **Answer with all of them, each with its failing case.** The example used to show exactly one
  finding, and an example is the part of a prompt a model copies hardest. It now shows three, in three
  severities, so neither the count nor the kind is what gets copied. The `detail` of each names the
  input or state and the wrong result that follows, and the prompt asks for that in words as well —
  which is the whole guard against an invented finding, and the reason none of this is a quota: a
  reviewer that cannot write down the failing case has generally found a suspicion, and is told to put
  it in its reasoning instead. Asked of `detail`, which is free text already, so nothing in the schema,
  the parser or the saved state moved to carry it. And "send an empty findings list when there is
  nothing to report" stays exactly as it was, because the other three turn into pressure the moment the
  empty answer stops being plainly allowed.

All four have a short form, and the sweep's descriptions, the gaps question and two of the example's
three entries are what give way when the prompt has to fit a Windows command line — measured, because
the full set put the floor prompt at 4 604 quoted characters against the 4 000 a short budget allows,
which would have stopped the run this tile exists to keep alive from starting at all. Off the command
line — every prompt handed over on stdin, which is every run on Claude Code — nothing is trimmed. The
gaps question is the one that is **dropped rather than shortened**: what makes it safe is the weighing
clause, and a short form would keep the invitation while spending its saving on the guard.
...
**Completion criteria are set on the tile** (`GoalCompletionCriteria`, persisted in `GoalTileState`, edited through `GoalCriteriaEditor` — its own view model, because running a workflow and editing a handful of settings are not the same job — panel behind the tune button — plain text boxes rather than `NumericUpDown`, because the panel is drawn as a config file and a spinner is the one control that would make it a dialog; the cost is that nonsense typed into a number field is ignored rather than refused, which the bounds at the point of use absorb — and where those bounds bite, the attempts row says so beside the field rather than snapping the value back, which would fight anyone typing "10" one digit at a time). Edits are **debounced and never written over nothing**: these are text boxes, so a handler runs per keystroke, and saving on each would both stutter and create — on a tile nobody has set a goal in yet — the empty session file in the user's repository that `Dispose` and `NewGoalAsync` both go out of their way to avoid. Criteria set before a goal exists live in memory until one does. The settings themselves: tolerated errors (0) and warnings (0), whether the review must say `goalMet` (yes), attempts (5, was a `const`), **which SOLID principles apply** (all five) and **whether the project has to build and its tests to pass** (both yes; see the paragraph below). The defaults reproduce the old behaviour for anyone who never opens the panel. Blockers have no tolerance and no field: a blocker is the finding that says the change is unacceptable rather than merely wrong, and a setting for those would only ever be used to ship something the reviewer said must not ship. **The project builds** and **its tests pass** are the two criteria that are not scored out of the review's findings: they are stated in the implement and review prompts, and the reviewer is told to establish them by running this project's own commands, because neither can be read off a diff. That last clause used to end "rather than by reading the diff", which was the only sentence in the whole review prompt naming an action for the reviewer to take — and the action it named excluded the code. A reviewer following it literally runs the build, reports it green and stops; see *What the review is asked to do* below. A failure the changes caused comes back as an `error` finding and is counted like any other; one that was already failing does not, and is reported in the reasoning instead.

**A review is not automatically an instruction — the gate between them** (`GoalReviewGatePolicy`, `GoalDismissals`, `GoalReviewGateMode`, the *after each review* row on the criteria panel). The loop used to hand its own findings straight back to the tool, so the one moment somebody can say "that one is not worth fixing" — the list on screen, nothing re-implemented over it yet — lasted about as long as it takes to read the first finding. Three modes, per goal and not per machine, because the decision is about this piece of work rather than about the user: **countdown** (the default, 15 s) shows the findings and carries on by itself when the clock runs out; **pause** stops after every review and waits for Resume, however long that takes; **off** is the old behaviour. The mode is stored in `GoalTileState`, read tolerantly like every enum there, and a goal file written before any of this existed reads as *countdown* rather than *off* — deliberately, since a gate nobody has switched on is a gate nobody uses.

**Touching a tick stops the clock, and it does not start again.** That is the rule the whole thing was asked for: somebody who has just decided one finding is not worth fixing is still reading the rest of the list, and a clock running underneath them re-implements the review they are in the middle of making up their mind about. The gate then behaves exactly as *pause* does, and the way on is the same Resume the rest of the tile uses rather than a second verb for the same act. A second tick does not restart it either — that would be a gate that can be held open only by fidgeting with it. The state machine is three functions (`Initial`, `AfterEdit`, `Expired`) and a sentence (`Line`), pure and argued in a table test for the reason `ChainPolicy` and `GoalLoopPolicy` are: the loop around it needs an AI process and a git worktree to turn over once.

**Every error and warning arrives ticked, and what stays ticked is what goes back.** The tick is on the finding's own row in the transcript, first in the row — everything to the right of it *describes* the finding, and a control the eye has to find among three kinds of text is a control that gets missed while a clock runs. Unticked is drawn dimmed and stays where it is: it is the record of a decision, and a reader two attempts later has to be able to see what was decided as well as what was found. **A blocker offers no tick at all** — that is the one severity with no tolerance in the completion criteria either, and a tick beside it would be that tolerance reached from somewhere else. And the gate itself does not open when there is nothing to pick (a review of blockers alone, or prose the parser could not structure) or when the budget is spent and the loop is about to summarise anyway — except in *pause*, where the user has said they want the lap to stop whatever it found.

**A suggestion is the exception, and the other way round: it arrives unticked.** Suggestions are never sent back and never counted, and ticked by default they would be neither of those any more — a column of nits the tool is suddenly asked to fix. So the tick beside one is an opt-in: left alone it is exactly what a suggestion always was (and it is not dimmed, since unticked says nothing new about it), and ticked it goes back to the tool with the errors and warnings and **holds the goal open while a review still raises it** — `GoalWorkflowEngine.IsMet` is the criteria plus that one rule, because the criteria themselves count no suggestion and must not start to. What is remembered is the yes (`IncludedSuggestions`, beside `DismissedFindings` in the goal file and cleared with it on a new goal), matched by `GoalFinding.Defect` like a dismissal. After a Review on its own, ticking a suggestion on a clean review is what turns a finished goal back into one Continue offers to work on. The loop's own gate still opens only on a review that did not meet the criteria — stopping every finished run for fifteen seconds to offer its nits would be the gate charging for the one case it exists not to hold up — so a run that ends as met over a review with suggestions offers their ticks on its summary instead, judged as a review on its own: ticking one is what offers Continue. When a ticked suggestion is the only thing holding the goal open, the reason given is that one (`GoalWorkflowEngine.WhyNotMet`), never the criteria's "not met".

**Unticked means not counted, not merely not mentioned.** `GoalDismissals.Accepted` subtracts what has been dismissed from the review, and the result is what the completion criteria are asked about, what the sentence saying why they were not met is built from, what goes back to the tool as feedback and what the no-progress stop fingerprints. One subtraction for all four, because two of them would disagree in the one way there is no recovering from: a dismissed error kept out of the prompt, so nothing ever touches it, *and* counted by the criteria, so the goal is refused for it on every lap until the budget is gone. The criteria are asked again the moment the gate closes — dismissing the last error is a goal that is finished, and taking that answer only on the next lap would be an attempt spent re-implementing against an empty list.

**The verdict is not one of the findings.** `GoalDismissals.Accepted` subtracts findings and leaves `goalMet` exactly as the reviewer wrote it, because the review prompt asks for the two separately and a `goalMet:false` can rest on something no finding states — half a feature, a case the goal named that the diff does not cover. There is no tick beside that, and there must be no way to reach it from one: carried over with the findings, a warning unticked about a variable name finishes a goal whose own review said the work was half done, and with `CommitWhenDone` commits it. What that costs is the run whose every stated defect has been dismissed and whose verdict still says no: it spends one lap on an empty list and stops as going round in a circle. That is the right answer rather than a rough edge — the reviewer is saying something the list never said, and the way out is Pause and a word in the composer, not a tick.

**What carries a dismissal across laps is the prompt, not a string match.** The reviewer is run from scratch every lap and remembers nothing, so it phrases the same defect differently each time; the text match — `GoalFinding.Defect`, severity, file and title, which `GoalDismissals.Contains` and `GoalReviewResult.Fingerprint` share — catches only the literal repeat. **The line is deliberately not in it**, in either of them and for one reason: both comparisons are made either side of an implementation, and an implementation moves the lines under every defect it did not fix. For the fingerprint that meant the no-progress stop never firing; for a dismissal it meant the same untouched defect coming back three lines lower as a finding nothing had ever dismissed — ticked, counted against the criteria and back in the feedback, with the user's decision undone in silence and no way to take it again except at the next gate. What the looser identity costs is one collision the line used to prevent: two findings of the same severity in one file under the *same title* are folded into one, so a tick beside either dismisses both. That is the better trade — the collision dismisses a second defect the user was already minded to leave in the same file, while the miss makes the tick useless exactly where it was asked for. The review prompt therefore carries the dismissed findings as a block saying *the user has decided not to fix these; do not report them again, in these words or in any others*, and the model reading it is the only part of the arrangement that can tell "dereferences null at 762" from "this can be null here". It is said as a decision rather than as a correction on purpose: a reviewer told that a real defect is not a defect argues with the file in front of it and raises it again in other words, which is the thing the block exists to stop. `GoalWorkflowEngine.BuildReviewPrompt` reads the list itself rather than taking it as a parameter, so no caller can build a review prompt that forgets it. Severity, place and title only — the detail is the longest part of a finding and the least useful for recognising one again.

**The dismissals belong to the goal.** They survive a restart, an approved plan replacing a rejected one (a rejected plan and its replacement are usually about the same defect, so clearing them there is how something unticked twenty seconds ago comes back on the first review of the new plan), and the run ending; they go with the goal being replaced. They are stored once, as whole findings on `GoalTileState.DismissedFindings`, and the tick on each finding in the transcript is derived from that list on load — a copy of the answer on every finding of every review would be a second record of one fact, and the way that fails is a finding shown as dismissed that nothing is actually subtracting. `GoalTileState.LastReview` keeps the review whole beside them, because a tick moved after the run has stopped has to rebuild the feedback the next implementation will get.

**A tile closed at the gate comes back at it.** The picks are reopened on load when the run is paused mid-loop, in the waiting state rather than the counting one — a countdown restored as counting would be a number on screen with no clock behind it. What closes the picks is the next implementation starting, which is the one moment the decision stops being changeable, and that is reached from `RunImplementReviewLoopAsync` rather than from wherever the pause was taken: a pause is exactly the case where the ticks have to stay.
...
**The status bar says nothing about the states something else already explains.** Four sentences came
...
plan, and a summary line over a summary. Each could only repeat the control beneath it, on a bar under the composer that
...
transcript, where it lands beside the spinner that says something is happening at all, and disappears with
it. The **paused** labels stay, and they are the exception that shows the rule: that is the one state
...
**A spinner says the tool is working.** A braille spinner turns in the gutter of the row at the foot of
the transcript, where the next message's glyph will stand, while `IsRunning` is set (`WaitingRow`). The
status bar under the composer already names the phase and the file being read; what it
...
a progress bar: that would be a claim about how far along the run is, which nothing here knows. The
spinner turns only while the row is effectively visible — a workspace switched away from keeps its
view attached and hidden, so there the timer drops to one look a second. Where the spinner sits, and why it is a
row rather than a badge, is two paragraphs below.

**Beside them, what the tool is doing and for how long** — `implementing · 2/5 · 4:07`, ticked once a
second by a `DispatcherTimer` that runs only while `IsRunning` is set. The spinner says something is
...
no label. Both are set in the transcript's own monospace, so nothing beside the spinner shuffles
sideways once a second as the digits change width.
...
gets a placeholder message — a braille spinner turning where the gutter marker goes, the stage and clock where the text will be —
...
it said a second time what the status bar under the composer already says, with the phase and the
...
**A check switched off is forbidden in words, not left out.** Silence is not an instruction: an agent
builds and runs the suite by reflex after a change, so a prompt that merely omits the tests has them run
anyway. `GoalPromptBuilder.SkippedChecks` says so outright, and the implement prompt always says
"Do not commit" — the review reads `git diff HEAD`, and a commit mid-goal takes the work out of it.

**When the tests run is a choice of its own** (`GoalCompletionCriteria.TestTiming`, the picker beside
the tests switch, rules in `GoalTestPolicy`, argued in a table test). *every review* is what the tile
always did: the implementer keeps them passing and every review checks. *when done* runs them only once
a review has accepted the work, as a step of its own (`GoalPromptBuilder.BuildTestRun`); a failure is an
error finding in the review's own shape, goes through the gate and the feedback like any other, and sends
the run round again, where the next acceptance runs them again. *first review + when done* adds them to
the first review, so a change that breaks the suite early is caught before later attempts are built on
it. In the deferred laps the prompts say *when* the tests run rather than forbidding them — and the
implementer keeps the tests a finding names, since the lap after a failed test run is spent fixing
exactly those. Two decisions worth keeping:

- **The owed step is not a second review.** A reviewer asked again rarely says the same thing twice, so
  an accepted goal would be reopened over a remark rather than a failing test. The step is told the code
  is judged and that anything but a failure belongs in its reasoning, and a structured answer counts as
  the goal met whatever it says about it.
- **Nothing is persisted for it.** A pause during the test step leaves the phase at Review, so Resume
  reviews again and, accepted, owes the tests again — one review more than strictly needed, against a
  flag in the goal file for a window a few minutes long. A second field rather than a four-way
  replacement of `RequireTestsPass` is the same economy: an old file reads as *every review*, and a
  rolled-back build ignores the key and runs them every review, never less.
...
**The composer is the Agent tile's, attachments included** (see *Images and files* in
[`AGENT-CONVERSATIONS.md`](AGENT-CONVERSATIONS.md)): each image and each file the text names has a chip above
the box, read off the text and removed together with its marker or mention (`ComposerEdit`), and a file that
is not a picture goes in as an `@` mention — copied into `.mtiles/attachments/` first when it lies outside the
workspace (`AttachmentStore`). **Such a file is something to read, never the scope of the goal**
(`GoalTileViewModel.ScopeMentions`, `AttachmentStore.IsContextOnly`): made a scope, it narrowed every read of
the tree to an ignored directory that never changes, so the review was handed nothing and passed work it had
...
Three strips: a strip on top (execution agent · mode · effort — a `DockPanel`, because a horizontal `StackPanel` measures with infinite width and `TextTrimming` there is inert), the transcript with a floating **Jump to bottom (Ctrl+End)** button over its foot (`JumpToBottom`), and a composer at the bottom with a status bar under it (status · activity · finding badges, the Agent tile's `context-bar`) drawn as one field with the prompt glyph inside it — the `Border` owns the border and focus ring, the `TextBox` gives up its own, and a click anywhere in it puts the caret in the box. Enter sends, Shift+Enter breaks a line. Buttons are the app-wide `tile-btn`. Pause/Resume appear while the tool is running; `+` starts a fresh goal.
