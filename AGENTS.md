...
- `Services/Tiles/` — the tile registry (see *Tiles* below and [`docs/TILES.md`](docs/TILES.md)): ITileKind, TileKind<T>, TileCatalog/TileCatalogEntry, TileContext, TileState, AgentStateKeys (the names a layout writes an agent-running tile's state down under, shared by the two kinds that do), and one class per kind (TerminalTileKind, TerminalAgentTileKind, NoteTileKind, TodoTileKind, GitTileKind, DatabaseTileKind, GoalTileKind, UsageTileKind, and the window's two permanent ones, WorkspacesTileKind and WorkspaceHostTileKind)
...
- `agent-conversations/conversations.db` — every Agent tile's conversation: its events and the id that
  ...
  create mode on the file would reach. Nothing prunes it yet
