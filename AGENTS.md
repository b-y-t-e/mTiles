...
**The cache and the list are the window's, and the window's layout only borrows them.** `MainWindow` builds the `WorkspacesPanelView` and the panel of cached workspace views once, and `CreateWindowTileView` hands each to a `WindowTileFrame` — no card, no header, a drop target and a hint overlay — for the tile that stands for it; every other window tile is an ordinary `LeafTileView`. Moving the list re-parents both controls rather than building either again, which is what keeps every workspace's shells alive through a rearrangement (`WindowLayoutViewTests`). The list is dragged by its heading, or by the grip the collapsed strip has instead, and its width is written back into `WorkspacesPanelWidth` on close so a rolled-back build opens it at that width.
