...
- **Providers have no page in Settings yet.** An agent instance can be pointed at Anthropic, OpenAI, OpenRouter, z.ai, LM Studio or Ollama — the code is there, the key is encrypted the way the database passwords are, and the model list, the compatibility check and the environment are all wired end to end — but nothing in Settings creates one, so today it takes editing `settings.json` by hand. The page is the last piece of the AI section's rework.
- **Settings cannot be exported or imported.** Moving to a second machine means copying `settings.json` and knowing which parts of it are encrypted to the first machine's account.
...
- **Settings export and import.** Everything mTiles knows about agents, providers and shells lives in one file, and moving to a second machine means finding it and copying it by hand. Export and import belong in the application. The one thing that must be decided rather than defaulted: **API keys are excluded unless explicitly asked for**, because an exported configuration is the kind of file people mail to themselves — and on Linux those keys are already stored as plain text (see *AI providers*), so an export carrying them is a plain-text secret in transit. Open questions: whether an import merges or replaces, and what happens to an instance whose agent or provider the importing machine does not have.
