...
/// <summary>
/// How typing into a picker narrows a long list.
/// </summary>
/// <remarks>
...
/// <para>Pure and in a class of its own rather than inline in the control, for the reason
...
/// <para><b>Ranked, it is not.</b> t3code scores its matches and sorts by the score; this keeps the
/// caller's order, because in every list here that order already means something — the conversations are
/// most-recent-first, the efforts run low to high, and a relevance sort would scramble both. A list long
/// enough to need ranking is a reason to revisit this, not a reason to sort three-row menus.</para>
/// </remarks>
public static class PickerSearch
{
    ...
    /// </summary>
    /// <remarks>An empty search matches everything, which is what lets a picker offer the whole
    ...
    /// <summary>Whether a row matches, over everything about it that is words.</summary>
    /// <remarks>The title, both lines under it and the caller's own keywords, joined and asked once — so
    /// typing a provider's name finds its models, which is t3code's rule and the reason its search box
    /// answers "claude" with the whole Claude family rather than nothing.</remarks>
    public static bool Matches(string? search, PickerOption option) =>
        Matches(search, string.Join(' ', new[]
        {
            option.Title, option.Detail, option.Description, option.Keywords,
